        ...
        {result && (
          <div className="animate-result-in space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Card padding="md" className="text-center"><p className="text-xs text-text-muted">Freigrenze</p><p className="text-xl font-bold font-currency text-accent-500 mt-1">{formatCurrency(result.pfaendungsfreigrenze)}</p></Card>
              <Card padding="md" className="text-center"><p className="text-xs text-text-muted">Pfändbar</p><p className="text-xl font-bold font-currency text-negative-500 mt-1">{formatCurrency(result.pfaendbarerBetrag)}</p></Card>
              <Card padding="md" className="text-center"><p className="text-xs text-text-muted">Verbleibend</p><p className="text-xl font-bold font-currency text-text mt-1">{formatCurrency(result.verbleibendesBetrag)}</p></Card>
            </div>
          </div>
        )}
      </div>
